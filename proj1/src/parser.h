...
void gather_decls(AST *ast, char *env_func, int is_top_level, int can_define_function); // i used a different meaning for is_top_level, basically if in function or not.
...
extern smap *stack_sizes; // to know how far to move $sp down and up later.
/** Maps from function names to the number of arguments they expect. */
extern smap *num_args; // to check tree shape is correct
/** Holds a reference to each of the different string literals appearing throughout 
 *  the program. */
extern smap *strings; // to know which string# it is in code_gen
